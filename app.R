...
###############################################################################################################################  
  ...
  clean_text_source <- reactive({
    req(tweet_source_tag())
    tweet_source_tag()%>%select(clean_text)
  })
...
###############################################################################################################################
  ...
  num_of_followers <- reactive({
    req(user_source_tag())
    (user_source_tag() %>% users_data() %>%unique() %>% select(followers_count))[[1]]
  })
  ...
  screen_name <- reactive({
    req(user_source_tag())
    (user_source_tag()%>%users_data()%>%unique()%>%select(name))[[1]]
  })
  ...
  user_min_date <- reactive({
    req(user_df())
    (user_df()%>%summarise(min(created_at)))[[1]]
  })
  
  # gets maximum date 
  user_max_date <- reactive({
    req(user_df())
    (user_df()%>%summarise(max(created_at)))[[1]]
  })
  ...
  clean_user_text_source <- reactive({
    req(user_source_tag())
    user_source_tag()%>%select(clean_text)
  })
...
###############################################################################################################################
  ...
  output$usermindate <- renderText({
    input$submit_submit
    isolate(prettyNum(user_min_date()))
  })
  
  
  output$usermaxdate <- renderText({
    input$submit_submit
    isolate(prettyNum(user_max_date()))
  })
