...
server <- function(input, output) {
  ...
  tweet_table_data <- reactive({
    req(tweet_df())
    tweet_df() %>%
      dplyr::select(created_at, text, favorite_count, retweet_count) %>%
      mutate(
        Tweet =text,
        URLs = "",
      ...
      dplyr::select(DateTime = created_at, Tweet, Likes = favorite_count, RTs = retweet_count)
  })
  ...
  user_positive_table <- reactive({
    ...
    if (input$variable_9 == 'likes'){
      ...
      user_source_tag()[order(-user_source_tag()$retweet_count),] %>%
        ...
        dplyr::select(DateTime = created_at, Tweet, Retweets = retweet_count)
    }
    else {
      user_source_tag()[order(-user_source_tag()$engagement),] %>%
        ...
        dplyr::select(DateTime = created_at, Tweet, Engagement = engagement)
    }
  })
  ...
  user_negative_table <- reactive({
    ...
    if (input$variable_10 == 'likes'){
      ...
      user_source_tag()[order(-user_source_tag()$retweet_count),] %>%
        filter(sentiment_tag=='negative') %>%
        dplyr::select(created_at, text, retweet_count) %>%
        mutate(
          Tweet =text,
          created_at = created_at %>% 
            # Remove zeros.
      ...
      user_source_tag()[order(-user_source_tag()$engagement),] %>%
        filter(sentiment_tag=='negative') %>%
        dplyr::select(created_at, text, engagement) %>%
        mutate(
          Tweet =text,
          created_at = created_at %>% 
            # Remove zeros.
